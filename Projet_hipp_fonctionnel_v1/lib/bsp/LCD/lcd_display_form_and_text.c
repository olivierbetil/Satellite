...
void LCD_DrawLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length,
		uint8_t Direction, uint16_t color) {
	uint32_t i = 0;
 ...
 *  *****************************************************
 */
bool_e LCD_DrawTriangle(uint16_t xref, uint16_t yref, uint16_t hauteur,
		uint16_t largeur, uint16_t color) {
	bool_e ret;
	uint16_t x, y;
	if ((0 < xref) && (xref < (320 - largeur)) && (0 < yref)
			&& (yref < (240 - hauteur))) {
		for (x = xref; x < (xref + largeur); x++) {
			for (y = yref + (x - xref); y < (yref + hauteur) - (x - xref);
					y++) {
 ...
 * \return None
 */
void LCD_WritePicture(uint16_t Xpos, uint16_t Ypos, uint8_t Height,
		uint16_t Width, const uint16_t *picture) {
	uint32_t index;
...
/*
 ...
 Octets par pixel : 2
 */
const uint8_t bmp_header[70] = { 0x42, 0x4D, 0x46, 0x58, 0x02, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x46, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x40, 0x01,
		0x00, 0x00, 0xF0, 0x00, 0x00, 0x00, 0x01, 0x00, 0x10, 0x00, 0x03, 0x00,
		0x00, 0x00, 0x00, 0x58, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8,
		0x00, 0x00, 0xE0, 0x07, 0x00, 0x00, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00 };
...
uint32_t get_BMP_width(uint8_t *BmpAddress) {
	return U32FROMU8(BmpAddress[21], BmpAddress[20], BmpAddress[19],
			BmpAddress[18]);
}
...
uint32_t get_BMP_height(uint8_t *BmpAddress) {
	return U32FROMU8(BmpAddress[25], BmpAddress[24], BmpAddress[23],
			BmpAddress[22]);
}
...
uint32_t get_BMP_data_offset(uint8_t *BmpAddress) {
	return U32FROMU8(BmpAddress[13], BmpAddress[12], BmpAddress[11],
			BmpAddress[10]);
}
 ...
 * @retval None
 */
void LCD_BMP_Resized(uint8_t *BmpAddress, uint16_t Xpos, uint16_t Ypos,
		uint8_t Split) {

	uint32_t index = 0, size = 0, width = 0, height = 0, index_fin = 0, compt_1,
			compt_2, width_cut, height_cut;
	uint16_t Valeur_R = 0, Valeur_G = 0, Valeur_B = 0, Valeur_Pixel = 0,
			Bytes_Per_Pixel;
	if (!LCD_is_initialized())
		return;
	/* Read bitmap size */
...
uint8_t LCD_miniBMP_Split_Calculate(uint8_t *BmpAddress) {
	if (get_BMP_width(BmpAddress) > 106 && get_BMP_height(BmpAddress) > 80) {
		if (((float) get_BMP_width(BmpAddress)
				/ (float) get_BMP_height(BmpAddress)) <= (float) 4 / 3)
...
void LCD_DrawFullRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width,
		uint16_t Height, uint16_t border_color, uint16_t inside_color) {
	if (!LCD_is_initialized())
		return;
	...
	while (Height--) {
		LCD_DrawLine(Xpos + 1, Ypos++, Width - 2, LCD_DIR_HORIZONTAL,
				inside_color);
	}
}
...
void LCD_DrawFullCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius,
		uint16_t border_color, uint16_t inside_color) {
	int32_t D; /* Decision Variable */
