...
void SPI_Init(SPI_TypeDef *SPIx) {
	...
	if (id == SPI1_ID) {
		...
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_5, GPIO_MODE_AF_PP, GPIO_PULLUP,
				GPIO_SPEED_HIGH, GPIO_AF5_SPI1);
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_6, GPIO_MODE_INPUT, GPIO_PULLUP,
				GPIO_SPEED_HIGH, GPIO_AF5_SPI1);
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_7, GPIO_MODE_AF_PP, GPIO_PULLUP,
				GPIO_SPEED_HIGH, GPIO_AF5_SPI1);
		...
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_13, GPIO_MODE_AF_PP, GPIO_PULLUP,
				GPIO_SPEED_HIGH, GPIO_AF5_SPI2);
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_14, GPIO_MODE_INPUT, GPIO_PULLUP,
				GPIO_SPEED_HIGH, GPIO_AF5_SPI2);
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_15, GPIO_MODE_AF_PP, GPIO_PULLUP,
				GPIO_SPEED_HIGH, GPIO_AF5_SPI2);
	}

	//Configuration of the SPIx module
	hSPI[id].Instance = SPIx;
	hSPI[id].Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_2;
...
void SPI_WriteAndReadNoRegister(SPI_TypeDef *SPIx, uint8_t *Rxdata,
		uint8_t Txdata) {
	assert(SPIx == SPI1 || SPIx == SPI2);
	SPI_ID_e id = ((SPIx == SPI2) ? SPI2_ID : SPI1_ID);
	HAL_SPI_TransmitReceive(&hSPI[id], &Txdata, Rxdata, 1, 100);
}

void SPI_WriteAndReadMultiNoRegister(SPI_TypeDef *SPIx, uint8_t *Rxdata,
		uint8_t *Txdata, uint16_t count) {
	assert(SPIx == SPI1 || SPIx == SPI2);
	SPI_ID_e id = ((SPIx == SPI2) ? SPI2_ID : SPI1_ID);
	HAL_SPI_TransmitReceive(&hSPI[id], Txdata, Rxdata, count, 100);
}
