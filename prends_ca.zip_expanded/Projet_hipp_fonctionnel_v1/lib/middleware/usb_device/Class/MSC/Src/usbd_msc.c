...
USBD_ClassTypeDef USBD_MSC = { USBD_MSC_Init, USBD_MSC_DeInit, USBD_MSC_Setup,
NULL, /*EP0_TxSent*/
NULL, /*EP0_RxReady*/
USBD_MSC_DataIn, USBD_MSC_DataOut,
NULL, /*SOF */
NULL,
NULL, USBD_MSC_GetHSCfgDesc, USBD_MSC_GetFSCfgDesc,
		USBD_MSC_GetOtherSpeedCfgDesc, USBD_MSC_GetDeviceQualifierDescriptor, };
...
__ALIGN_BEGIN uint8_t USBD_MSC_CfgHSDesc[USB_MSC_CONFIG_DESC_SIZ] __ALIGN_END
		...
		LOBYTE(MSC_MAX_HS_PACKET), HIBYTE(MSC_MAX_HS_PACKET), 0x00, /*Polling interval in milliseconds */
...
__ALIGN_BEGIN uint8_t USBD_MSC_OtherSpeedCfgDesc[USB_MSC_CONFIG_DESC_SIZ] __ALIGN_END
		...
		MSC_EPIN_ADDR, /*Endpoint address (IN, address 1) */
		0x02, /*Bulk endpoint type */
		0x40, 0x00, 0x00, /*Polling interval in milliseconds */
...
__ALIGN_BEGIN uint8_t USBD_MSC_DeviceQualifierDesc[USB_LEN_DEV_QUALIFIER_DESC] __ALIGN_END
		...
		USB_DESC_TYPE_DEVICE_QUALIFIER, 0x00, 0x02, 0x00, 0x00, 0x00,
...
uint8_t USBD_MSC_Setup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req) {
	USBD_MSC_BOT_HandleTypeDef *hmsc =
			(USBD_MSC_BOT_HandleTypeDef*) pdev->pClassData;
	...
	case USB_REQ_TYPE_CLASS:
		...
		case BOT_GET_MAX_LUN:
			...
			if ((req->wValue == 0) && (req->wLength == 1)
				...
				hmsc->max_lun =
						((USBD_StorageTypeDef*) pdev->pUserData)->GetMaxLun();
