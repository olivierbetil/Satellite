...
static int8_t SCSI_Inquiry(USBD_HandleTypeDef *pdev, uint8_t lun,
	...
	if (params[1] & 0x01)/*Evpd is set*/
	{
		...
		pPage =
				(uint8_t*) &((USBD_StorageTypeDef*) pdev->pUserData)->pInquiry[lun
...
static int8_t SCSI_ReadCapacity10(USBD_HandleTypeDef *pdev, uint8_t lun,
	...
	if (((USBD_StorageTypeDef*) pdev->pUserData)->GetCapacity(lun,
			&hmsc->scsi_blk_nbr, &hmsc->scsi_blk_size) != 0) {
		SCSI_SenseCode(pdev, lun,
		NOT_READY,
		MEDIUM_NOT_PRESENT);
		return -1;
...
static int8_t SCSI_ReadFormatCapacity(USBD_HandleTypeDef *pdev, uint8_t lun,
	...
	if (((USBD_StorageTypeDef*) pdev->pUserData)->GetCapacity(lun, &blk_nbr,
			&blk_size) != 0) {
		SCSI_SenseCode(pdev, lun,
		NOT_READY,
		MEDIUM_NOT_PRESENT);
...
static int8_t SCSI_Read10(USBD_HandleTypeDef *pdev, uint8_t lun,
	...
	if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
	{
		...
		hmsc->scsi_blk_addr = (params[2] << 24) | (params[3] << 16)
				| (params[4] << 8) | params[5];
...
static int8_t SCSI_Write10(USBD_HandleTypeDef *pdev, uint8_t lun,
	...
	if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
	{
		...
		if (((USBD_StorageTypeDef*) pdev->pUserData)->IsWriteProtected(lun)
				!= 0) {
			SCSI_SenseCode(pdev, lun,
			NOT_READY,
			WRITE_PROTECTED);
...
static int8_t SCSI_ProcessRead(USBD_HandleTypeDef *pdev, uint8_t lun) {
	...
	MSC_EPIN_ADDR, hmsc->bot_data, len);
